<template>
    <div>
        <div class="modal fade editPackageModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    ...
                    <div class="card-body m-t-35">
                        <form name="packages" method="post" class="form-horizontal login_validator" id="form_inline_validator">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-xl-9">
                                    <featureDetail  v-for="n in keyValuePairCount" :data="details[n-1]" :key="n" @update-key="updateKey" @update-value="updateValue" />
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" @click="adminPackageUpdate()">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        mounted () {
            EventBus.$on('package-edit-data-fetch', (payload) => {
                this.id = payload
                axios.get('package/edit/' +payload)
                .then(response => {
                        var packages = response.data
                        this.title = packages.title
                        this.service = packages.service
                        this.duration = packages.duration
                        this.price = packages.price
                        this.orders = packages.orders
                        this.icon = packages.icon
                        this.details = packages.details
                        this.keyValuePairCount = packages.details.length
        ...
        methods: {
            ...
            adminPackageUpdate () {
                ...
                axios.post('package/update/' +this.id, {
                    title: this.title,
